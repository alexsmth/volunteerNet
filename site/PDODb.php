 ...
 * @aut/or    Josh Campbell <jcampbell@ajillion.com>
...
class PDODb
{
    ...
    public function get($tableName, $numRows = null, $columns = '*')
    {
        ...
        $columns = $this->santitizeQuery($columns);
    ...
    /**
     * To sanitize the query to avoid the issue I discovered above CUSTOM added by alex
     *  @param array or String $fields The array of columns or the table name that you would like to sanitize
     *  @return array or String $sanitizedFields The array of sanitized columns or the sanatized Table name 
     */
    public function santitizeQuery($fields)
    {
        if (is_array($fields)) {
                foreach($fields as $i) {
                    $arr = (strpos($i, '.') != false) ? explode('.',$i) : $i;
                    if (is_array($arr)) {
                        foreach($arr as $o){
                             $o = '`'.str_replace('`','',$o).'`';
                         }
            ...
            $fields = ($fields != '*') ?  '`'.str_replace('`','',$fields).'`' : $fields;
            return $fields;
        }
